...
export default function AdminCompanies() {
  ...
  const [rejectModal, setRejectModal] = useState<{ show: boolean; companyPk: number | null }>({ show: false, companyPk: null });
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      const usersBody = usersRes.data as { error?: boolean; data?: Array<Record<string, unknown>> };
      const pendingBody = pendingRes.data as { data?: Array<{ id: number; user_id: number; submitted_at?: string; company_sector?: string; town?: string }> };
      ...
      const mapped: Company[] = usersBody.data.map((u: Record<string, unknown>) => {
        ...
        return {
          ...
          description: "",
          logo: null,
          status: !active ? "suspended" : isPending ? "pending" : ("approved" as Company["status"]),
  ...
  const confirmReject = async () => {
    ...
    try {
      await api.post(`admin/companies/${rejectModal.companyPk}/reject/`, { reason: rejectReason });
      setCompanies((prev) =>
        prev.map((c) =>
      ...
      setCompanies((prev) =>
        prev.map((c) =>
          c.userId === userId
            ? {
                ...c,
                status: activate ? ("approved" as const) : ("suspended" as const),
              }
            : c,
        ),
      );
    ...
    <div className="am-page-root">
      ...
      <div className="am-companies-grid">
        {filtered.map(company => (
          <div key={company.userId} className={`am-company-card ${company.status === "pending" ? "am-company-pending" : ""}`}>
            ...
            <div className="am-company-card-footer">
              ...
              {company.status === "pending" && (
                <div className="am-pending-actions">
                  <button onClick={() => handleApprove(company.companyPk)} className="am-btn-approve-sm">✓ Approve</button>
                  <button onClick={() => setRejectModal({ show: true, companyPk: company.companyPk })} className="am-btn-reject-sm">✕ Reject</button>
                </div>
              )}
              {company.status === "approved" && (
                <button onClick={() => handleToggleActive(company.userId, company.status)} className="am-btn-suspend-sm">⏸ Suspend</button>
              )}
              {company.status === "suspended" && (
                <button onClick={() => handleToggleActive(company.userId, company.status)} className="am-btn-reactivate-sm">▶ Reactivate</button>
              )}
            </div>
          </div>
        ))}
      ...
      {/* Reject Modal */}
      {rejectModal.show && (
        <div className="am-modal-overlay" onClick={() => setRejectModal({ show: false, companyPk: null })}>
          <div className="am-modal" onClick={e => e.stopPropagation()}>
            <div className="am-modal-head">
              <h3>Reject Company</h3>
              <button onClick={() => setRejectModal({ show: false, companyPk: null })} className="am-close-btn">✕</button>
            </div>
            <div className="am-modal-body">
              ...
              <div className="am-form-actions">
                <button onClick={() => setRejectModal({ show: false, companyPk: null })} className="am-btn-cancel">Cancel</button>
            ...
            <div className="am-modal-body">
              ...
              {selectedCompany.status === "pending" && (
                <div className="am-detail-actions-bottom">
                  <button onClick={() => { handleApprove(selectedCompany.companyPk); setShowDetailModal(false); }} className="am-btn-approve">✓ Approve Company</button>
                  <button onClick={() => { setShowDetailModal(false); setRejectModal({ show: true, companyPk: selectedCompany.companyPk }); }} className="am-btn-reject">✕ Reject Company</button>
                </div>
              )}
            </div>
          </div>
