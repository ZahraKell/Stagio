...
from .models import Administration, Student, Company, SignupOTP
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = User
        fields = ['username', 'password', 'confirm_password', 'email', 'full_name', 'role']
    ...
    def validate(self, attrs):
        ...
        # Auto-generate username from email local-part if omitted.
        requested_username = (attrs.get('username') or '').strip()
        base_username = requested_username or email.split('@')[0]
        if not requested_username:
            # Matches your signup UX: local part + 4 random digits.
    ...
    def create(self, validated_data):
        ...
        if role == 'student':
            ...
            Company.objects.create(user=user, is_approved=False, is_rejected=False)
        elif role == 'administration':
            Administration.objects.create(user=user)

        SignupOTP.create_for_user(user)
        return user
