...
def list_companies(request):
    if not is_admin(request.user):
        return fail("Admins only.", status.HTTP_403_FORBIDDEN)
    companies = Company.objects.select_related('user').order_by('-submitted_at', '-id')
    data = []
    for c in companies:
        if c.is_approved:
    ...
    return ok(data=data, message=f"{len(data)} company profile(s).")
